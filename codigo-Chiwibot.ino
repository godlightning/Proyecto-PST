...
//------------------ Valores de PID 
double Kp = 60;  // Kp = 60; 
double Kd = 3.5;  // Kd = 2.2;  3.5
double Ki = 270;  // Ki = 270;  250
...
void setup()
{
Serial_2.begin(9600);    // inicia el puerto serial para comunicación con el Bluetooth
 //Serial.begin(9600);  
...
void loop()
{
 ...
 //********************************************** Fin Control Blu ************************ 
 // si el programa falla, detiene toda acción
 if (!dmpReady) return;

 // espera el accionar del MPU
 while (!mpuInterrupt && fifoCount < packetSize)
 {
 //si no hay información del MPU, calcula el PID para los motores 
 pid.Compute();
